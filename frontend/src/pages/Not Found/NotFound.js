...
const NotFound = () => {
  ...
  // Lightweight parallax/tilt retained for visual polish
  useEffect(() => {
    ...
    const handleMove = (e) => {
      ...
      lastX = (x / rect.width) * 2 - 1;
      lastY = (y / rect.height) * 2 - 1;
    ...
    <div ref={containerRef} className={styles.container} data-theme={theme}>
      ...
      <div ref={cardRef} className={styles.card} role="main" aria-labelledby="nf-title">
        ...
        <div className={styles.content}>
          <div className={styles.gameHeader} style={{ marginTop: 8 }}>We couldn't find that page.</div>
          <p className={styles.title} style={{ marginTop: 8 }}>Try returning home or check the URL.</p>
        </div>
      </div>
